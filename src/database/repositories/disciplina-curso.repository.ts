...
export type IDisciplinaCursoRepository = ReturnType<
  typeof getDisciplinaCursoRepository
>;
...
export const getDisciplinaCursoRepository = (
  ...
  dataSource.getRepository(DisciplinaCursoDbEntity).extend({
    async updateDisciplinaCurso(
      payload: Partial<DisciplinaCursoDbEntity>,
      id: number,
    ) {
      const updatedData = await this.createQueryBuilder('disciplina_curso')
        .update<DisciplinaCursoDbEntity>(DisciplinaCursoDbEntity, {
          ...payload,
        })
