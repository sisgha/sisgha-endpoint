...
export type ITurmaHasTurnoAulaRepository = ReturnType<
  typeof getTurmaHasTurnoAulaRepository
>;
...
export const getTurmaHasTurnoAulaRepository = (
  ...
  dataSource.getRepository(TurmaHasTurnoAulaDbEntity).extend({
    async updateTurmaHasTurnoAula(
      payload: Partial<TurmaHasTurnoAulaDbEntity>,
      id: number,
    ) {
      const updatedData = await this.createQueryBuilder('turma_has_turno_aula')
        .update<TurmaHasTurnoAulaDbEntity>(TurmaHasTurnoAulaDbEntity, {
          ...payload,
        })
